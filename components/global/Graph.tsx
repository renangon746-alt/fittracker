...
import { PesoPoint } from '@/hooks/auth/useProfileStats';
...
function formatFecha(iso: string, t: (k: string) => string) {
    ...
    return (
        <View style={[{ borderRadius: 24, padding: 14, backgroundColor: colors.backgroundPrimary }, SHADOW]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <View>
                    <Text style={{ fontSize: 11, color: colors.textSecondary, marginBottom: 1 }}>{t('your_weight')}</Text>
                    <Text style={{ fontSize: 24, fontWeight: '700', letterSpacing: -0.8, color: colors.textPrimary }}>
                        {pesoActual ?? '––'}{' '}
                        <Text style={{ fontSize: 14, fontWeight: '400', color: colors.textSecondary }}>{t('kg')}</Text>
                    </Text>
                </View>
            </View>
