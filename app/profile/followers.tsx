...
import { fetchFollowers, FollowUser } from '@/hooks/social/useFollowers';
...
export default function FollowersScreen() {
  ...
  useEffect(() => {
    ...
    fetchFollowers(targetId).then(list => {
      setUsers(list);
      setLoading(false);
    });
  }, [id, targetId]);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.backgroundSecondary }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 10, gap: 6 }}>
        ...
        <Text style={{ fontSize: 18, fontWeight: '700', color: colors.textPrimary }}>{t('followers')}</Text>
      </View>
